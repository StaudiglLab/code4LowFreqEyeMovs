...
% Run this after automatic saccade detection (SacDetect_Exp1)
%% load data
codepath = '';
...
% select the subject to do visual inspection
sub = 1;
...
bl_len_thr = 100.*(600/1000); % more than 100 ms of NaNs chunk
for s = sub
...
%%
range_dva = [-14,14];% upper and lower limit of gaze position (unit in dva) from the centre to plot
time_range = [-1.3,4];% [-1.3, 4] sec locked to image presentation
...
for s = sub
    ...
    DPP_x = visAngPerPixel(stimparami.xlen, cfg.screen.dist, stimparami.hPxl);% calculate visual angle per pixel based on participant's distance to the screen
    DPP_y = visAngPerPixel(stimparami.ylen, cfg.screen.dist, stimparami.vPxl);
    
    
    %% plot  
    for i_trial = 25:length(data_raw_left(s).trial)   
        ...
        % plot left eye data on the upper panel
    ...
    %% save
    save([sacsavepath,sprintf('Sub%d_ET_cleansaccades_%s.mat',s,'Exp1')],'movement_cleanblink_L','movement_cleanblink_R','badmovs4biggerintervals_L','badmovs4biggerintervals_R','blink_mark','trl_rej_ind','trl_rej_Eye_L','trl_rej_Eye_R','-v7.3');
end
