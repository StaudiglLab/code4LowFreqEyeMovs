%% Saccade detection for Exp 3 (Step 1)
...
savename = [datapath,'ET_Exp3.mat'];
load(savename)
%% Detect (micro)Saccades for Picture Viewing 
tp_epoch = [-1.3,4];
tp_inst = [-1.3,4];
...
cfg.velocity2D.velthres =3;%threshold for velocity outlier detection 
ft_warning off
for s = 1:length(FixPET_all)
    disp(s)
    for itrial = 1:length(data_ET_right(s).trial)
...
n_spthr = 6; % Max number of sample points with NaNs that allow to be interpolated 
% add constraint, no more than 6 NaNs in a row is allowed
...
%% save output
save([datapath,'ET_clean_saccades_Exp3.mat'],'data_ET_right','data_ET_left','MS_counter_right','MS_counter_left','movement_clean_right','movement_clean_left','-v7.3');
