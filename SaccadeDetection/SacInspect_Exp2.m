%% Visual inspection of detected saccade
% Run this after automatic saccade detection (SacDetect_Exp2)
%% load data
codepath = '';
...
% blink auto mark (chunks of NaNs)
...
for s = sub
    ...
    DPP_x_s1 = visAngPerPixel(stimparami.xlen, cfg.screen.dist, stimparami.hPxl);
    DPP_y_s1 = visAngPerPixel(stimparami.ylen, cfg.screen.dist, stimparami.vPxl);
    ...
    DPP_x_s2 = visAngPerPixel(stimparami.xlen, cfg.screen.dist, stimparami.hPxl);
    DPP_y_s2 = visAngPerPixel(stimparami.ylen, cfg.screen.dist, stimparami.vPxl);
 
    %% plot  
    
    for i_trial = 1:length(data_raw_left(s).trial)   
        clearvars DPP_x DPP_y
        % session 1 & 2
        if itrial<(length(data_raw_left(s).trial)/2+1)
            DPP_x = DPP_x_s1;
            DPP_y = DPP_y_s1;
        else
            DPP_x = DPP_x_s2;
        ...
        blink_line = (get(ax1, 'BrushData')); % marked on the ref line (blinks or not MS)
  ...
  %% save
    save([sacsavepath,sprintf('Sub%d_ET_cleansaccades_%s.mat',s,'Exp2')],...
...
for s=sub
    disp(s)
    clearvars  data_s  data_o
    data_o = cat(3,ET_all_eyelink(s).pvon{:});
    data_s = permute(data_o(:,tp_select(1):tp_select(2),:),[2,1,3]);
    data_s(data_s==0) = nan;

    for itrial = 1:size(data_s,3)
        data_raw_left(s).trial{1,itrial} = data_s(:,[1:2,3],itrial)';
        data_raw_left(s).time{1,itrial} = tp_inst(1):1/1000:tp_inst(2);
        
        data_raw_right(s).trial{1,itrial} = data_s(:,[4:5,6],itrial)';
        data_raw_right(s).time{1,itrial} = tp_inst(1):1/1000:tp_inst(2);
...
% blink auto mark (chunks of NaNs)
bl_len_thr = 50.*(1000/1000); % more than 50 ms of NaNs chunk
for s = sub
    for itrial = 1:length(data_raw_left(s).trial)
        Nandata_L = isnan(sum(data_raw_left(s).trial{1,itrial},1));
        Nandata_R = isnan(sum(data_raw_right(s).trial{1,itrial},1));
...
time_point = time_range(1):1/1000:time_range(2);
...
for s = sub
    ...
    for i_trial = 1:length(data_raw_left(s).trial)   
        ...
        % change to dva from center
        tmp_r(1,:) =  tmp_r(1,:)*DPP_x-cfg.screen.wc(1)*DPP_x;
        tmp_r(2,:) =  tmp_r(2,:)*DPP_y-cfg.screen.wc(2)*DPP_y;
        tmp_r(3,:) =  tmp_r(3,:)/1000-( max(tmp_r(3,:)/1000)-range_dva(2)); % up to 5
        tmp_interpo_r(1,:) = tmp_interpo_r(1,:)*stimparami.hPxl*DPP_x-cfg.screen.wc(1)*DPP_x;
        tmp_interpo_r(2,:) = tmp_interpo_r(2,:)*stimparami.vPxl*DPP_y-cfg.screen.wc(2)*DPP_y;
        
        tmp_l(1,:) =  tmp_l(1,:)*DPP_x-cfg.screen.wc(1)*DPP_x+diff(range_dva)+2;
        tmp_l(2,:) =  tmp_l(2,:)*DPP_y-cfg.screen.wc(2)*DPP_y+diff(range_dva)+2;
        tmp_l(3,:) =  tmp_l(3,:)/1000-( max(tmp_l(3,:)/1000)-range_dva(2))+diff(range_dva)+2; % 
        tmp_interpo_l(1,:) = tmp_interpo_l(1,:)*stimparami.hPxl*DPP_x-cfg.screen.wc(1)*DPP_x+diff(range_dva)+2;
        tmp_interpo_l(2,:) = tmp_interpo_l(2,:)*stimparami.vPxl*DPP_y-cfg.screen.wc(2)*DPP_y+diff(range_dva)+2;
        ...
        movement_L = movement_clean_left{s}{i_trial};
        if ~isempty(movement_L)
            movs2plot=(movement_L+tp_inst(1)*1000)./1000;          
            % add ms
            for m=1:size(movs2plot,1)
                xline(movs2plot(m,1),'LineStyle','--','Color',[0.9,0.9,0.9])
                v1 = [[movs2plot(m,1); movs2plot(m,2); movs2plot(m,2); movs2plot(m,1)], [y_range(1)+diff(range_dva)+2; y_range(1)+diff(range_dva)+2; y_range(2); y_range(2)]];
        ...
        movement_R = movement_clean_right{s}{i_trial};
        if ~isempty(movement_R)
            movs2plot=(movement_R+tp_inst(1)*1000)./1000;          
            % add ms
            for m=1:size(movs2plot,1)
                xline(movs2plot(m,1),'LineStyle','--','Color',[0.9,0.9,0.9])
                v1 = [[movs2plot(m,1); movs2plot(m,2); movs2plot(m,2); movs2plot(m,1)], [y_range(1); y_range(1); y_range(1)+diff(range_dva); y_range(1)+diff(range_dva)]];
        ...
        blk_auto = blink_automark{s}{i_trial};
        if ~isempty(blk_auto)
            movs2plot=(blk_auto+tp_inst(1)*1000)./1000;          
        ...
        blink_line = (get(ax1, 'BrushData')); % marked on the ref line (blinks or not MS)
        ms_line_L = (get(axL, 'BrushData'));
        ms_line_R = (get(axR, 'BrushData'));
        trl_rej = (get(axRej, 'BrushData'));
        trl_rej_L = (get(axRej_L, 'BrushData'));
        trl_rej_R = (get(axRej_R, 'BrushData'));
  ...
  %% save
   save([sacsavepath,sprintf('Sub%d_ET_cleansaccades_%s.mat',s+2,'Exp2')],...
    'movement_cleanblink_L','movement_cleanblink_R','badmovs4biggerintervals_L','badmovs4biggerintervals_R',...
    'blink_mark','trl_rej_ind','trl_rej_Eye_L','trl_rej_Eye_R','-v7.3');
end
