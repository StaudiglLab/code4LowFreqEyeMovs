%% Saccade detection for Exp 1 (Step 1)
...
savename = [datapath,'ET_Exp1.mat'];
load(savename)

%% Detect Saccades for Picture Viewing 
tp_epoch = [-1.3,4];
tp_inst = [-1.3,4];
...
for s=1:length(ET_all)
    ...
    data_s(:,1,:) = data_o(tp_select(1):tp_select(2),1,:)*1920;% from ratio to N of pixels
    ...
    for itrial = 1:size(data_s,3)
        ...
        %% Marking nans (start and end of NaNs, and register its position)
...
% velocity can thus be estimated for entire epoch, saccades overlapping
% with NaNs will be excluded later
for s = 1:length(ET_all)
    disp(s);
...
cfg.velocity2D.velthres =6;%threshold for velocity outlier detection
ft_warning off
for s = 1:length(ET_all)
    disp(s)
    for itrial = 1:length(data_ET_right(s).trial)
...
%% Kick out the saccades that coincide with NaNs and Outside of the area

n_spthr = 6; % Max number of sample points with NaNs that allow to be interpolated (~10 ms), this helps to deal with missing data points.
for s = 1:length(NaN_register_left)
...
for s = 1:length(NaN_register_right)
...
% exclude saccades outside the image size (28 x 21 dva)

for s = 1:length(data_ET_right)
    disp(s)
    %% load screen parameter for each subject
...
save([datapath,'ET_clean_saccades_Exp1.mat'],'data_ET_right','data_ET_left','MS_counter_right','MS_counter_left','movement_clean_right','movement_clean_left','-v7.3');
