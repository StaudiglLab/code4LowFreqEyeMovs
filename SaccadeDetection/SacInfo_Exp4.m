%% Saccade info (Step 4)
% Run this after Automatic detection of saccades (SacDetect_Exp4) and
% visual inspection (SacInspect_Exp4)
codepath = '';
...
load([datapath,'ET_clean_saccades_Exp4.mat']); clearvars movement_clean_left movement_clean_right
% get pupil
tp_epoch = [-1.3,4];
tp_inst = [-1.3,4];
...
for s=1:length(Mem)
    ...
    for itrial = 1:size(data_s,3)
        data_raw_left(s).trial{1,itrial} = data_s(:,[1:2],itrial)';
        data_raw_left(s).time{1,itrial} = tp_inst(1):1/1000:tp_inst(2);
        
        data_raw_right(s).trial{1,itrial} = data_s(:,[3:4],itrial)';
...
for s = 1:length(Mem)
    ...
    %% save
    savename = [sacsavepath,sprintf('Sac_PV_Info_Sub%d_%s.mat',s,'Exp4')];
