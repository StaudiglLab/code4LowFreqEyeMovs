%% Saccade info (Step 3)
% Run this after Automatic detection of saccades (SacDetect_Exp2) and
% visual inspection (SacInspect_Exp2)
codepath = '';
...
load([datapath,'ET_clean_saccades_Exp2_tb.mat']); clearvars movement_clean_left movement_clean_right ET_all_eyelink
...
load(savename)
Mem = Mem(1:2);Confi = Confi(1:2); clearvars pr_cond pr_perf pv_perf
%% Filter Sac with criteria
% 1: take out the MS in marked rej trials 
for s = 1:2
    disp(s)   
   load([sacsavepath,sprintf('Sub%d_ET_cleansaccades_%s.mat',s,'Exp2')])
    

   %% remove saccade in rejected eye
   ...
   %%
    ...
    % exclude trials with un marked eye having more than 67% of nans
...
% 2: take out MS xx ms before and after blink
...
% Interpolate nans for amplitude and direction calculation 
...
for AGL = 1
    for s=1:2
        ...
        for i_trl = 1:length(trl_ind_bi)
            ...
            if trl_idx<(length(data_ET_left(s).trial)/2+1)
                dist = dist_s1;
            else
                dist = dist_s2;    
            end
...
for s = 1:2
    ...
    %% save
    savename = [sacsavepath,sprintf('Sac_PV_Info_Sub%d_%s.mat',s,'Exp2')];
...
for el = 1
    t_sep = 0; % 0ms clear of upcoming ms
t_min = 50; % in ms free of the next saccades, to not count same saccade multiple times
for ms=1
load([datapath,'ET_Exp2.mat'])
load([datapath,'ET_clean_saccades_Exp2_el.mat']); clearvars movement_clean_left movement_clean_right ET_all_tobii
...
savename = [datapath,'Beh_Exp2.mat'];
load(savename)
Mem = Mem(3:4);Confi = Confi(3:4); clearvars pr_cond pr_perf pv_perf
...
for s=1:2
    for i_trl = 1:length(MS_bi{s})
        ...
        if ~isempty(MS_L{s}{i_trl})
            ms = MS_L{s}{i_trl};
            ms = ms(ms(:,1)>base_l*1000,:);
            ms = ms(ms(:,1)<base_end*1000,:);
            MS_L_pvon{s}{i_trl} =  ms;
        end
...
for s = 1:2
    ...
    for itrial = 1:length(ms_bi_trlind{s})
            ...
            MS_dir{i_trl} = ms_angl_bi{s}{itrial};
            MS_amp{i_trl} = ms_amp_bi{s}{itrial};
            %MS_dur{i_trl} = diff(MS_bi_pvon{s}{ms_bi_trlind{s}(itrial)},[],2)'*1/1000*1000;
            confi_rt(i_trl) = Confi{s}(ms_bi_trlind{s}(itrial));
            trl_number(i_trl) = ms_bi_trlind{s}(itrial); % current trial
    ...
    Sac_trials.trialinfo = Sac_trials.trialinfo(ro_idx,:);
    %% save
    savename = [sacsavepath,sprintf('Sac_PV_Info_Sub%d_%s.mat',s+2,'Exp2')];
    save(savename,'Sac_trials','-v7.3')
end
