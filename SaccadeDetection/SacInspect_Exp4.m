%% Visual inspection of detected saccade (Exp 4)
% mark the false positive saccades 
% Run this after automatic saccade detection (SacDetect_Exp4)
%% load data
codepath = '';
...
%%
sub = 31; % which subject to do

tp_epoch = [-1.3,4];
tp_inst = [-1.3,4];
tp_select = dsearchn((tp_epoch(1):1/1000:tp_epoch(2))',[tp_inst(1),tp_inst(2)]');
ext_tp = 100; % after nan detection, mark 100ms before and after nan also as nan (to take out blink residual)
for s=sub
    disp(s)
    ...
    %% replace here data -100 to 100 of nan also as nan
...
bl_len_thr = 50.*(1000/1000); % more than 100 ms of NaNs chunk
for s = sub
...
%%
range_dva = [-15,15];% dva range for visualization
time_range = [-1.3,4];
...
for s = sub
  ...
  %% save
    save([sacsavepath,sprintf('Sub%d_ET_cleansaccades_%s.mat',s,'Exp4')],...
