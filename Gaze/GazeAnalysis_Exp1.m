...
effect1 = computeCohen_d(MemMore_Sac(:,1),MemFewer_Sac(:,1),'paired');
effect2 = computeCohen_d(MemMore_Sac(:,2),MemFewer_Sac(:,2),'paired');
effect3 = computeCohen_d(MemMore_Sac(:,1),MemMore_Sac(:,2),'paired');
effect4 = computeCohen_d(MemFewer_Sac(:,1),MemFewer_Sac(:,2),'paired');
...
save([datafigspath,'SuppleFig9_right11.mat'],'stat_EI','Gaze_MoreExplr','Gaze_LessExplr')
%% Figure
...
Eye_trlinfo(:,9) = Img_id_enc{s}(Eye_trlinfo(:,1));
    %% sep by In vs Outdoor
    
    Gaze_Indoor(:,:,s) = nanmean(g_data(:,:,Eye_trlinfo(:,9)<=162 & Eye_trlinfo(:,8)==1),3);
    Gaze_outdoor(:,:,s) = nanmean(g_data(:,:,Eye_trlinfo(:,9)>162 & Eye_trlinfo(:,8)==1),3);
    
    
    %% sep by In/ Outdoor x Memory with balancing N of trials 

    [Gaze_InRem_balance(:,:,s),Gaze_InFor_balance(:,:,s),n_trl] = Gaze_balance_trial(g_data(:,:,Eye_trlinfo(:,2)==1 & Eye_trlinfo(:,8)==1 & Eye_trlinfo(:,9)<=162),g_data(:,:,Eye_trlinfo(:,2)==0 & Eye_trlinfo(:,8)==1& Eye_trlinfo(:,9)<=162),100);
    n_trl_InRF_balance(s,1:2) = n_trl;
    
    [Gaze_OutRem_balance(:,:,s),Gaze_OutFor_balance(:,:,s),n_trl] = Gaze_balance_trial(g_data(:,:,Eye_trlinfo(:,2)==1 & Eye_trlinfo(:,8)==1 & Eye_trlinfo(:,9)>162),g_data(:,:,Eye_trlinfo(:,2)==0 & Eye_trlinfo(:,8)==1& Eye_trlinfo(:,9)>162),100);
    n_trl_OutRF_balance(s,1:2) = n_trl;
...
subset = 1:20; % all subjects
for hitvsmis = 1
ave_2D_C1 = Gaze_Indoor;
ave_2D_C2 = Gaze_outdoor;
viewdva_x =28;
viewdva_y =21;
%%
Dat_C1 = [];
...
subset = 1:20; % all subjects
for hitvsmis = 1
ave_2D_C1 = Gaze_InRem_balance;
ave_2D_C2 = Gaze_InFor_balance;
viewdva_x =28;
viewdva_y =21;
%%
Dat_C1 = [];
...
[stat_CB_In]  = ft_freqstatistics(cfg, Dat_C1,Dat_C2);
...
subset = 1:20; % all subjects
for hitvsmis = 1
ave_2D_C1 = Gaze_OutRem_balance;
ave_2D_C2 = Gaze_OutFor_balance;
viewdva_x =28;
viewdva_y =21;
%%
Dat_C1 = [];
...
[stat_CB_Out]  = ft_freqstatistics(cfg, Dat_C1,Dat_C2);
...
save([datafigspath,'SuppleFig2_2.mat'],'stat_CB*','Gaze_Indoor','Gaze_outdoor','Gaze_OutRem_balance','Gaze_OutFor_balance','Gaze_InRem_balance','Gaze_InFor_balance')
%% Figure
codepath = '';
datafigspath =  [codepath,'\data4figs\'];
figsavepath = [codepath,'\figures\'];
% load behavioral data, gaze data
load( [datafigspath,'SuppleFig2_2.mat'])
...
fig_savename1 = 'SuppleFig2A_bottomright';
for hitvsmis=1
f=figure('Name',int2str(11),'units','normalized','outerposition',sizefig1);clf;
Cluster=zeros(n_bin_y,n_bin_x);
Cluster(squeeze(stat_CB.mask)) = 1;
...
c = colorbar;
c.Label.String = 't value';
title('Gaze density Indoor vs Outdoor')
...
fig_savename1 = 'SuppleFig2B_topright';
for hitvsmis=1
f=figure('Name',int2str(11),'units','normalized','outerposition',sizefig1);clf;
Cluster=zeros(n_bin_y,n_bin_x);
Cluster(squeeze(stat_CB_In.mask)) = 1;
[B,L,N,A] = bwboundaries(Cluster); 
imagesc(squeeze(stat_CB_In.stat));hold on
for k=1:length(B)
     boundary = B{k};
    plot(boundary(:,2), boundary(:,1), 'LineWidth',3,'color','k');
end
...
c = colorbar;
c.Label.String = 't value';
title('Gaze density Indoor: Rem vs Forg')
xlabel('Horizontal [dva]' )
ylabel('Vertical [dva]' )
set(findobj(gcf,'type','axes'),'FontName','Arial','FontSize',fontaxis,'FontWeight','normal', 'LineWidth', line_width);
...
fig_savename1 = 'SuppleFig2B_bottomright';
for hitvsmis=1
f=figure('Name',int2str(11),'units','normalized','outerposition',sizefig1);clf;
Cluster=zeros(n_bin_y,n_bin_x);
Cluster(squeeze(stat_CB_Out.mask)) = 1;
[B,L,N,A] = bwboundaries(Cluster); 
imagesc(squeeze(stat_CB_Out.stat));hold on
for k=1:length(B)
     boundary = B{k};
    plot(boundary(:,2), boundary(:,1), 'LineWidth',3,'color','k');
end
...
c = colorbar;
c.Label.String = 't value';
title('Gaze density Outdoor: Rem vs Forg')
xlabel('Horizontal [dva]' )
ylabel('Vertical [dva]' )
set(findobj(gcf,'type','axes'),'FontName','Arial','FontSize',fontaxis,'FontWeight','normal', 'LineWidth', line_width);
