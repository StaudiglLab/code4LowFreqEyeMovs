...
for s = 1:length(gaze2D)
    ...
    Gaze_LessExplr(:,:,s) = nanmean(g_data(:,:,Eye_trlinfo(:,8)>=cri & Eye_trlinfo(:,9)==1),3);
     %% Median split saccade
    cri1 = nanmedian(Eye_trlinfo(Eye_trlinfo(:,9)==1&Eye_trlinfo(:,4)==1,6));
    cri2 = nanmedian(Eye_trlinfo(Eye_trlinfo(:,9)==1&Eye_trlinfo(:,4)==2,6));
    cri3 = nanmedian(Eye_trlinfo(Eye_trlinfo(:,9)==1&Eye_trlinfo(:,4)==3,6));
   
    Gaze_MoreSac(:,:,s) = nanmean(g_data(:,:,((Eye_trlinfo(:,6)>=cri1 & Eye_trlinfo(:,4)==1)|(Eye_trlinfo(:,6)>=cri2 & Eye_trlinfo(:,4)==2)|(Eye_trlinfo(:,6)>=cri3 & Eye_trlinfo(:,4)==3)) & Eye_trlinfo(:,9)==1),3);
    Gaze_FewerSac(:,:,s)= nanmean(g_data(:,:,((Eye_trlinfo(:,6)<=cri1 & Eye_trlinfo(:,4)==1)|(Eye_trlinfo(:,6)<=cri2 & Eye_trlinfo(:,4)==2)|(Eye_trlinfo(:,6)<=cri3 & Eye_trlinfo(:,4)==3)) & Eye_trlinfo(:,9)==1),3);
...
save([datafigspath,'SuppleFig9_right41.mat'],'stat_EI','Gaze_MoreExplr','Gaze_LessExplr')
save([datafigspath,'SuppleFig10B_left.mat'],'stat_Sac','Gaze_MoreSac','Gaze_FewerSac')
%% Figure
...
%%
load([datafigspath,'SuppleFig10B_left.mat'])
...
sizefig1 = [0,0.2,0.3,0.45];
%
fig_savename1 = 'SuppleFig10B_left';
for hitvsmis=1
f=figure('Name',int2str(11),'units','normalized','outerposition',sizefig1);clf;
Cluster=zeros(n_bin_y,n_bin_x);
...
imAlpha = ones(size(squeeze(stat_Sac.stat)));
imAlpha(isnan(squeeze(stat_Sac.stat))) = 0;
imagesc(squeeze(stat_Sac.stat),'AlphaData',imAlpha);
hold on
for k=1:length(B)
     boundary = B{k};
    plot(boundary(:,2), boundary(:,1), 'LineWidth',3,'color','k');
