%% Saccade locked alpha/beta amplitude analysis (Exp4)
codepath = '';
...
for s = 1:34
    ...
    Hilbert_AlphaBeta{s} = ft_preprocessing(cfg,eeg_final_finetuned); %% 
...
t_eye = -1.3:1/1000:4; % time of saccade information
...
%% save output
save([datapath,'AlphaBeta_hilbert_Sac_real_Exp4.mat'],'AlphaBeta_Sac_locked','-v7.3'); 
save([datapath,'AlphaBeta_hilbert_Sac_shuffle_Exp4.mat'],'AlphaBeta_Sac_locked_rand','-v7.3'); 
%% Separate by conditions
load([datapath,'AlphaBeta_hilbert_Sac_real_Exp4.mat']);
load([datapath,'AlphaBeta_hilbert_Sac_shuffle_Exp4.mat'])
...
save([datafigspath,'SuppleFig3D.mat'],'Sac_amp_Memory','peak_latency_mem','peak_ampitude_mem','AlphaBeta_Rem','AlphaBeta_For')
%% Figure (Fig 5D)
codepath = '';
...
scatter(Sac_offset_amp(:,1)+300,repmat(-0.25,size(Sac_offset_amp,1),1),'filled','MarkerFaceColor',[ 52 148 186]./255,'LineWidth',1);
scatter(Sac_offset_amp(:,2)+300,repmat(-0.275,size(Sac_offset_amp,1),1),'filled','MarkerFaceColor',[193 128  219]./255,'LineWidth',1);
scatter(Sac_offset_amp(:,3)+300,repmat(-0.3,size(Sac_offset_amp,1),1),'filled','MarkerFaceColor',[193 219 128  ]./255,'LineWidth',1);hold off
...
figsavepath = [codepath,'\figures\'];
% load data
load( [datafigspath,'SuppleFig3D.mat'])
...
fig_savename = 'SuppleFig3D_right';
for fig=33
...
% scatter plots of both
sizefig = [0.1,0.1,0.35,0.6];
fig_savename = 'SuppleFig3D_left1';
...
fig_savename = 'SuppleFig3D_left2';
for fig=21
    %% 
    f=figure('Name',int2str(fig),'units','normalized','outerposition',sizefig);clf;
...
fig_savename = 'SuppleFig3D_left3';% Peak latency
for fig=21
    %% 
    f=figure('Name',int2str(fig),'units','normalized','outerposition',sizefig);clf;
