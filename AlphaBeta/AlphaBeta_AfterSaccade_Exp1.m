%% alpha/beta amplitude following a saccade (Exp1)
codepath = '';
...
for s = 1:20
    ...
    Hilbert_AlphaBeta{s} = ft_preprocessing(cfg,eeg_final); %% angles/phases
...
%% save output
save([datapath,'AlphaBeta_hilbert_AfterSac_Exp1.mat'],'AlphaBeta_Sac_locked','all_other_sac','-v7.3');
%%
...
t_range =  0:1/1000:1;
idx_tsig = dsearchn(t_range',stat_With.time(stat_With.mask)');
...
for s =  1:length(AlphaBeta_Sac_locked)
    ...
    AlphaBeta_RemNoPosSac(s,:) = mean(Datpos(AlphaBeta_Sac_locked(s).sacinfo(:,9)==0 & AlphaBeta_Sac_locked(s).sacinfo(:,2)==1,:));
    AlphaBeta_RemWithPosSac(s,:) = mean(Datpos(AlphaBeta_Sac_locked(s).sacinfo(:,9)~=0 & AlphaBeta_Sac_locked(s).sacinfo(:,2)==1,:));    
    
    %%
    % overall memory
    Avg_ampRF(s,1) = mean(mean(Datpos(AlphaBeta_Sac_locked(s).sacinfo(:,2)==1,idx_tsig)));
    Avg_ampRF(s,2) = mean(mean(Datpos(AlphaBeta_Sac_locked(s).sacinfo(:,2)==0,idx_tsig)));
...
for runstat = 1
    ...
    AlphaBeta_C1.avg = permute(AlphaBeta_RemWithPosSac(subset,:),[3,2,1]);
    AlphaBeta_C1.time = t_range;
    AlphaBeta_C1.dimord = 'chan_time_subj';
    AlphaBeta_C1.label = {'Pos'};
    AlphaBeta_C2 = AlphaBeta_C1;
    AlphaBeta_C2.avg = permute(AlphaBeta_RemNoPosSac(subset,:),[3,2,1]);
    ...
    [stat_Rem]  = ft_timelockstatistics(cfg, AlphaBeta_C1,AlphaBeta_C2);
end
%% save

save([datafigspath,'SuppleFig15.mat'],'stat_Rem','AlphaBeta_RemNoPosSac','AlphaBeta_RemWithPosSac','Avg_ampRF')
%% Figure

datafigspath =  [codepath,'\data4figs\'];
figsavepath = [codepath,'\figures\'];
% load data
load( [datafigspath,'SuppleFig15.mat'])
...
plot_areaerrorbar(AlphaBeta_RemWithPosSac(subset,toi_idx(1):toi_idx(2)),options);hold on
options.color_area = [219 193 128  ]./255;    % 
options.color_line = [ 186 148 52  ]./255;

plot_areaerrorbar(AlphaBeta_RemNoPosSac(subset,toi_idx(1):toi_idx(2)),options);hold on
for i_c=1:length(stat_Rem.posclusters)
    if stat_Rem.posclusters(i_c).prob<0.025
        sig_tw = stat_Rem.time(stat_Rem.posclusterslabelmat==i_c);
        t_idx= dsearchn(t_range',sig_tw');
        v1 = [[t_idx(1); t_idx(end); t_idx(end); t_idx(1)], [ylim_plt(1); ylim_plt(1); ylim_plt(2); ylim_plt(2)]];
        f1 = [1 2 3 4];
        patch('Faces',f1,'Vertices',v1,'FaceColor','black','FaceAlpha',.1);
...
for i_c=1:length(stat_Rem.negclusters)
    if stat_Rem.negclusters(i_c).prob<0.025
        sig_tw = stat_Rem.time(stat_Rem.negclusterslabelmat==i_c);
        t_idx= dsearchn(t_range',sig_tw');
        v1 = [[t_idx(1); t_idx(end); t_idx(end); t_idx(1)], [ylim_plt(1); ylim_plt(1); ylim_plt(2); ylim_plt(2)]];
        f1 = [1 2 3 4];
        patch('Faces',f1,'Vertices',v1,'FaceColor','black','FaceAlpha',.1);
...
xlabel('Time [ms]')
ylabel('Alpha Beta amplitude')
title('Posterior Alpha Beta (Rem)')
xlim([1,length(toi_idx(1):toi_idx(2))])
ylim(ylim_plt)
...
fig_savename = 'SuppleFig15A';
for fig=22
    %% 
    f=figure('Name',int2str(fig),'units','normalized','outerposition',sizefig);clf;
    h=daboxplot([Avg_ampRF(:,[1:2])],[repmat(1,size(Avg_ampRF,1),1)],...
        'scatter' ,2,...
        'color',cb,...
        'boxalpha', 0.5,...
        'jitter',1,...
