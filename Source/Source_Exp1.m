...
for s = 1:20
    ...
    %% N saccade
    cri = nanmedian(eeg_all.trialinfo(:,5));
    
    % More Sac
    cfg= [];
    cfg.latency = bl;
    cfg.trials = eeg_all.trialinfo(:,5)>=cri;
    dataPre_MoreSac  = ft_selectdata(cfg, eeg_all);
    
   
    cfg.latency = [1 4];
    cfg.trials = eeg_all.trialinfo(:,5)>=cri;
    dataPost_MoreSac = ft_selectdata(cfg,eeg_all);
    ...
    cfg.latency = bl;
    cfg.trials = eeg_all.trialinfo(:,5)<=cri;
    dataPre_LessSac  = ft_selectdata(cfg, eeg_all);
    
   
    cfg.latency = [1 4];
    cfg.trials = eeg_all.trialinfo(:,5)<=cri;
    dataPost_LessSac = ft_selectdata(cfg,eeg_all);
    
    n_trl_MoreLessSac(s,1) = sum(eeg_all.trialinfo(:,5)>=cri);
    n_trl_MoreLessSac(s,2) = sum(eeg_all.trialinfo(:,5)<=cri);
    clearvars cri
    ...
    cfg.tapsmofrq = taps;
    cfg.foilim    = foi_freq;
    freqPre_MoreSac = ft_freqanalysis(cfg, dataPre_MoreSac);
    freqPost_MoreSac = ft_freqanalysis(cfg, dataPost_MoreSac);
    freqPre_LessSac = ft_freqanalysis(cfg, dataPre_LessSac);
    freqPost_LessSac = ft_freqanalysis(cfg, dataPost_LessSac);  
    ...
    dataAll_MoreSac = ft_appenddata([], dataPre_MoreSac, dataPost_MoreSac);
    dataAll_LessSac = ft_appenddata([], dataPre_LessSac, dataPost_LessSac);
    ...
    freqAll_MoreSac = ft_freqanalysis(cfg, dataAll_MoreSac);
    freqAll_LessSac = ft_freqanalysis(cfg, dataAll_LessSac);
    ...
    sourceAll_MoreSac = ft_sourceanalysis(cfg, freqAll_MoreSac);
    sourceAll_LessSac = ft_sourceanalysis(cfg, freqAll_LessSac);
    ...
    cfg.sourcemodel.filter = sourceAll_MoreSac.avg.filter;
    sourcePre_MoreSac  = ft_sourceanalysis(cfg, freqPre_MoreSac );
    sourcePost_MoreSac = ft_sourceanalysis(cfg, freqPost_MoreSac);
    
    cfg.sourcemodel.filter = sourceAll_LessSac.avg.filter;
    sourcePre_LessSac  = ft_sourceanalysis(cfg, freqPre_LessSac );
    sourcePost_LessSac = ft_sourceanalysis(cfg, freqPost_LessSac);
    ...
    sourceDiff_MoreSac = sourcePost_MoreSac;
    sourceDiff_MoreSac.avg.pow = ( sourcePost_MoreSac.avg.pow -sourcePre_MoreSac.avg.pow) ./ sourcePre_MoreSac.avg.pow;
    
    sourceDiff_LessSac = sourcePost_LessSac;
    sourceDiff_LessSac.avg.pow = ( sourcePost_LessSac.avg.pow -sourcePre_LessSac.avg.pow) ./ sourcePre_LessSac.avg.pow;
    ...
    s_DiffInt_MoreSac{s}  = ft_sourceinterpolate(cfg, sourceDiff_MoreSac , mri);
    s_DiffInt_LessSac{s}  = ft_sourceinterpolate(cfg, sourceDiff_LessSac , mri);
...
cfg.alpha       = 0.025; 
cfg.tail        = 0;
stat_Sac   = ft_sourcestatistics(cfg, s_DiffInt_MoreSac{subset}, s_DiffInt_LessSac{subset});

%% save output
stat_RF = rmfield(stat_RF,'cfg');% save space
stat_Sac = rmfield(stat_Sac,'cfg');
save([datafigspath,'Fig1C_right.mat'],'stat_RF')
save([datafigspath,'Fig1E_right.mat'],'stat_Sac')
...
load( [datafigspath,'Fig1C_right.mat'])
load( [datafigspath,'Fig1E_right.mat'])
